...
index = 0
dxl0_goal_position = [1000, 100]   #1000 released, 100 closed
dxl_goal_position = [1000, 100]   #1000 released, 100 closed
dxl2_goal_position = [800, 2375]   #rotate limits
dxl3_goal_position = [550, 1500]  #550 released, 1500 closed
...
dxl_addparam_result = groupBulkRead.addParam(DXL0_ID, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)
if dxl_addparam_result != True:
    print("[ID:%03d] groupBulkRead addparam failed" % DXL0_ID)
    quit()
...
while 1:
    ...
    param_goal_position0 = [DXL_LOBYTE(DXL_LOWORD(dxl0_goal_position[index])), DXL_HIBYTE(DXL_LOWORD(dxl0_goal_position[index])), DXL_LOBYTE(DXL_HIWORD(dxl0_goal_position[index])), DXL_HIBYTE(DXL_HIWORD(dxl0_goal_position[index]))]
    ...
    param_goal_position2 = [DXL_LOBYTE(DXL_LOWORD(dxl2_goal_position[index])), DXL_HIBYTE(DXL_LOWORD(dxl2_goal_position[index])), DXL_LOBYTE(DXL_HIWORD(dxl2_goal_position[index])), DXL_HIBYTE(DXL_HIWORD(dxl2_goal_position[index]))]
    param_goal_position3 = [DXL_LOBYTE(DXL_LOWORD(dxl3_goal_position[index])), DXL_HIBYTE(DXL_LOWORD(dxl3_goal_position[index])), DXL_LOBYTE(DXL_HIWORD(dxl3_goal_position[index])), DXL_HIBYTE(DXL_HIWORD(dxl3_goal_position[index]))]

    # Add Dynamixel#0 goal position value to the Bulkwrite parameter storage
    dxl_addparam_result = groupBulkWrite.addParam(DXL0_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position0)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupBulkWrite addparam failed" % DXL0_ID)
        quit()
    ...
    dxl_addparam_result = groupBulkWrite.addParam(DXL3_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position3)
    if dxl_addparam_result != True:
    ...
    while 1:
        ...
        if (not (abs(dxl_goal_position[index] - dxl1_present_position) > DXL_MOVING_STATUS_THRESHOLD)) and (not (abs(dxl3_goal_position[index] - dxl3_present_position) > DXL_MOVING_STATUS_THRESHOLD)) and (not (abs(dxl2_goal_position[index] - dxl2_present_position) > DXL_MOVING_STATUS_THRESHOLD)) and (not (abs(dxl_goal_position[index] - dxl0_present_position) > DXL_MOVING_STATUS_THRESHOLD)):
            break
